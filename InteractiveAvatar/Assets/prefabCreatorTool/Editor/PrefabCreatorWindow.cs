...
namespace prefabCreatorTool.Editor
{
    public class PrefabCreatorWindow : EditorWindow
    {
        ...
        **/
            ...
            scrollPosTarget = EditorGUILayout.BeginScrollView(scrollPosTarget);
            {
                EditorGUILayout.BeginVertical();
                {
                    EditorGUILayout.BeginHorizontal();
                    {
                        ...
                        baseRigObject =
                            EditorGUILayout.ObjectField(baseRigObject, typeof(GameObject), true) as GameObject;
                    }
                    ...
                    if (GUILayout.Button("Create Instance in Scene"))
                        createPrefab();
                    GUILayout.Label(
                        "Make sure to drag this instance from Hierarchy to Project to create a prefab if you want to use this specific mix in another scene");
                }
            }
            ...
            foreach (GameObject item in itemList)
            {
                ...
                for (int c = itemMeshes.Length - 1; c > -1; c--)
                {
                    ...
                    else
                    {
                        ...
                        Dictionary<string, Transform> newRootBonesMap =
                            createNameToObjectMap<Transform>(
                                baseRigObj.transform.GetComponentsInChildren<Transform>(true));
        ...
        public void transferBones(SkinnedMeshRenderer rend, Transform newRoot)
        {
            ...
            Dictionary<string, Transform> newRootBonesMap =
                createNameToObjectMap<Transform>(newRoot.GetComponentsInChildren<Transform>(true));
            ...
            for (int ndx = 0; ndx < numBones; ++ndx)
            {
                if (newRootBonesMap.ContainsKey(originalBones[ndx].name))
                    ...
                    Debug.LogWarning("transferBones, failed to find key (" + originalBones[ndx].name +
                                     ") in dictionary, this is probably breaking your characters!");
            }

            rend.bones = outBones;
            rend.quality =
                SkinQuality.Bone4; //forces the quality level which fixes some skinning issues (dont trust auto) 
        }
